...
try {
  ...
  const result = await metered.generateText({ task: "text", modelRole: "FAST", system: "Follow the user's exact output instruction.", messages: [{ role: "user", content: "Return exactly the word ZAYLOQ_READY." }], maxOutputTokens: 16 }, { organizationId: organization.id, userId: user.id, operationType: "OPENAI_SMOKE", idempotencyKey: key, reservationUnits: BigInt(10_000_000), provider: "OPENAI", model: config.AI_FAST_MODEL });
